...
{
	struct i2c_pnx_algo_data *alg_data = dev_get_drvdata(dev);
	return PNXclkEnable(alg_data->clk);
}
...
{
	unsigned long tmp, freq;
	int ret = 0;

	struct resource *res;
	u32 speed = I2C_PNX_SPEED_KHZ_DEFAULT * 1000;

	struct i2c_pnx_algo_data *alg_data = pdev->dev.platform_data;
	if (!alg_data) {
#ifdef CONFIG_OF
...
#else
		dev_err(&pdev->dev, "%s: missing platform data\n", __func__);
		return -EINVAL;
	}
#endif
	...
	alg_data->timeout = alg_data->timeout ?: I2C_PNX_TIMEOUT_DEFAULT;
	...
	ret = PNXclkEnable(alg_data->clk);
	if (ret)
		return ret;

	freq = PNXclkRate(alg_data->clk);
	...
	alg_data->irq = alg_data->irq ?: platform_get_irq(pdev, 0);
	if (alg_data->irq < 0) {
